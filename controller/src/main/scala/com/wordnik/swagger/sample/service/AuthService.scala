...
class AuthService extends AuthDialog with ClientValidator {
  def validate[T](authorizationCode: String, f: => T):T = {
    Cache.tokenCache.contains(authorizationCode) match {
      case true => {
        val token = Cache.tokenCache(authorizationCode)
  ...
  def login(request: HttpServletRequest, response: HttpServletResponse) = {
    ...
    if(scope == "anonymous") {
      if(isValidRedirectUri(redirectUri)) {
        ...
        Cache.tokenCache += accessToken -> TokenWrapper(new Date, token)
        val redirectTo = {
    ...
    if(isValidUser(username, password) && accept.toLowerCase != "deny") {
      if(isValidRedirectUri(redirectUri)) {
        // it's good to proceed
        val oauthIssuerImpl = new OAuthIssuerImpl(new MD5Generator())
        val accessToken = oauthIssuerImpl.accessToken()
  ...
  def token(request: HttpServletRequest, 
    ...
    try {
      ...
      if(isValidClient(oauthRequest.getParam(OAuth.OAUTH_CLIENT_ID), oauthRequest.getParam(OAuth.OAUTH_CLIENT_SECRET))) {
        if("authorization_code" == grantType) {
          if(!Cache.codeCache.contains(code)){
